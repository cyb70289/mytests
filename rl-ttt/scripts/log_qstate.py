...
Produces a JSON file with per-episode snapshots of Q-values for multiple
canonical board states. Runs phase 1a + phase 1b training via the train
module with a per-episode callback.

Usage:
    python scripts/log_qstate.py --out web/qstate-data.json --seed 0

The output JSON is consumed by web/qstate-animation.html.
"""
...
def main() -> int:
    ...
    p.add_argument("--out", type=str, default="web/qstate-data.json",
