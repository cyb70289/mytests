...
# View the animation (serve the web/ directory, or open the HTML directly):
...
main play page.  The data loads via a plain `<script>` tag so the page
also works when opened directly from `file://` (no HTTP server needed).
...
│   └── qstate-data.js       # gitignored; generated by `python scripts/log_qstate.py`
